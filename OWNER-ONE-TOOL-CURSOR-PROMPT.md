...
- **타겟: 모든 자영업자 대상 금지.** 반드시 **업종 중심** 설계(랜딩·키워드·카피·기능 노출).
...
| Background | `#F9FAFB` |
| Foreground | `#111827` |
| 타겟 | **업종별 1차 타겟** (아래 타겟 업종 전략 참고). 모든 자영업자 대상 ❌ |

### ⚠ 타겟 업종 전략 — 진짜 중요 (이거 안 하면 실패함)

- **모든 자영업자 대상 ❌** — 절대 아님. 한 번에 다 잡으려 하면 메시지·기능이 흐려져 실패함.
- **Cursor 설계는 반드시 업종 중심**으로 해야 함.
  - 랜딩/SEO: 업종별 키워드·Pain Point·혜택 문구를 **업종마다 구분**해서 작성.
  - UI/기능 노출: 가능하면 업종(또는 진단 시 선택 업종)에 따라 **다른 메시지·우선 기능** 노출.
  - 슬로건·카피: "자영업자 올인원"보다 **"○○ 업종 사장님을 위한"** 같은 업종 구체화 권장.
- **1차 타겟 업종** (PMF 우선): 공간 대여(파티룸·렌탈스튜디오·공유오피스), 반려동물 서비스, 교육·클래스, 뷰티·에스테틱(네일·헤어·왁싱), 주문 제작(수제케이크·타투 등). 상세는 `docs/STRATEGY_PMF_AND_OPS.md` 참고.
